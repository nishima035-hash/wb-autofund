...
Для первого запуска по IP, когда порты 80/443 свободны, выполните `bash deploy_nginx.sh`. Панель станет доступна по публичному IP сервера. Перед боевым использованием рекомендуется подключить домен и HTTPS.
